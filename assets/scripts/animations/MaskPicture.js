...
export default class MaskPicture {
    constructor({ $view }) {
        ...
        this.$container.addEventListener('mousemove', (e) => (this.cursor = [e.x, e.y - this.top]))
    ...
    render() {
        ...
        if (this.circle.canRender) {
            ...
            const y = this.cursor[1] + ScrollManager.scroll

            this.circle.x += (x - this.circle.x) * 0.1
            this.circle.y += (y - this.circle.y) * 0.1

            const size = this.circle.size * 0.5

            this.ctx.beginPath()
            this.ctx.arc(this.circle.x, this.circle.y, size * this.circle.ratio, 0, 2 * Math.PI)
            this.ctx.fill()
            this.ctx.closePath()

            const imgX = this.circle.x - size
            const imgY = this.circle.y - size
